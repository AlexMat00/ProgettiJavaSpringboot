...
import com.nttdata.esercitazione.CorsoRepository;
...
*/
    ...
    public ResponseEntity<?> createCorso(@RequestBody Corso corso) {
        ...
        if (b) {
            return new ResponseEntity<>("Errore duplicato",
    ...
    public ResponseEntity<?> getCampi() {
        ...
        String campiC = null;
        String sc;
        for(Corso c: lista){sc = c.toString();
            campiC = campiC +sc;}
        return new ResponseEntity<>(campiC , HttpStatus.OK);
    ...
    public ResponseEntity<?> getCourses() {
        ...
        for(Corso corso: corsi){
            if(corso.getDescrizioneCorso().length() > 20){
                corsi1.add(corso);
            }
        }
